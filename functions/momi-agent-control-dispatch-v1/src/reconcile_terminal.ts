...
export async function reconcileTerminal(
  ...
  const execute = issue.teamLabels.find((label) => label.name === "execute-run")
  const hasRun = issue.teamLabels.find((label) => label.name === "has-run")
  if (!execute || !hasRun) throw new Error("linear_action_labels_unavailable")
  const labels = issue.labels.filter((label) => label.id !== execute.id)
  ...
  const work = { work_id: terminal.work_id, issue_id: context.issue_id,
    issue_identifier: context.issue_identifier, issue_url: "", project_id: null,
